...
void update(const sf::Vector2f &mousePosition, Eye &eyeL, Eye &eyeR)
{
    const sf::Vector2f maxAppleRadius = {50.f, 105.f};
    ...
    sf::Vector2f posAppleL = {
        maxAppleRadius.x * std::sin(-eyeL.rotation + float(M_PI) / 2),
        maxAppleRadius.y * std::cos(-eyeL.rotation + float(M_PI) / 2)
    };
    sf::Vector2f posAppleR = {
        maxAppleRadius.x * std::sin(-eyeR.rotation + float(M_PI) / 2),
        maxAppleRadius.y * std::cos(-eyeR.rotation + float(M_PI) / 2)
    };
